...
def config():
    ...
    if not current_user and form.picture.data:
...
def search():
    ...
    if request.method == "POST":
        ...
        no_results = not users  # Bandera para comprobar si hay resultados
        return render_template('search_results.html', users=users, search_query=search_query, theme=theme, no_results=no_results)
